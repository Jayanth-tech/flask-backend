...
async def upload_video(video: UploadFile = File(...)):
    ...
    asyncio.create_task(process_video(input_path, output_path, csv_path))
    return {"status": "success", "message": "Processing started"}

# async def process_video(input_path: str, output_path: str, csv_path: str):
...
async def process_video(input_path: str, output_path: str, csv_path: str):
    ...
    while True:
        ...
        if frame_count % 7 == 0:  # Skip 4 frames and process every 5th frame
            try:
                results = MODEL(frame, conf=0.25)[0]
